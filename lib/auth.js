...
function createAuth({ prisma = defaultPrisma, createSupabaseClient = createDefaultSupabaseClient } = {}) {
  ...
  async function attachCurrentUser(req, res, next) {
    ...
    res.locals.identityVerified = false;
    ...
    try {
      ...
      res.locals.identityVerified = hasVerifiedIdentity(req.currentUser);
      return next();
    } catch (error) {
      return next(error);
    }
  ...
  function requireVerifiedIdentity(req, res, next) {
    ...
    if (hasVerifiedIdentity(req.currentUser)) return next();

    const nextPath = safeRedirect(req.verificationNextPath || req.originalUrl, "/min-bruker");
    const params = new URLSearchParams({ next: nextPath });

    return res.redirect(`/bankid/verifiser?${params.toString()}`);
  }
  ...
  function requireAdmin(req, res, next) {
    ...
    if (!req.currentUser) {
      return res.redirect(`/admin/logg-inn?${new URLSearchParams({
        error: "Du må logge inn som administrator.",
      }).toString()}`);
    }
  ...
  async function signUpWithPassword({ email, password, fullName, emailRedirectTo, res }) {
    if (!isConfigured()) {
      throw new Error(getConfigurationErrorMessage());
    }
...
module.exports.hasVerifiedIdentity = hasVerifiedIdentity;
